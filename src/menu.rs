...
pub fn display_menu() -> bool {
	let mut current_selection = menu_option::Start; // by default, "Start" will be selected first
	let reader = Reader;
	...
	loop {
		...
		// change selected option with arrow keys, break loop with ENTER
		match event {
			KeyEvent {
				code: KeyCode::Up,
	...
	// depending on the selection before breaking with ENTER, either return true or false
	match current_selection {
